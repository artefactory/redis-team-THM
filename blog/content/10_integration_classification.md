...
- `re-training`: This script [`scripts/retrain_model.sh`]({filename}../../scripts/retrain_model.sh)  retrains the model, it needs to be run if new papers are added to the dataset. As a next step, it would be good to create a [Cloud Function](https://cloud.google.com/functions) in order to automate the retraining of our model if new data is detected.


# Including the soft labels and the text encoding in redis

We decided to have only one [entry point]({filename}../../scripts/generate_index.py) to generate the meta data of every paper:
...
The final pickle file is then pushed to `redis` using [`load_data.py`]({filename}../../scripts/load_data.py).
