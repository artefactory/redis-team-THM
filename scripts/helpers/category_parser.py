...
def parse_classifier_prediction(prediction_dicts: Union[Dict, List[Dict]], categories_list: List[str], confidence_threshold: float) -> str:
    ...
    for prediction in prediction_dicts:
        if prediction["score"] >= confidence_threshold:
            ...
            prediction_strings.append(f"{prediction_category}({prediction['score']:0.4f})")
