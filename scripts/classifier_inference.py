...
from thm.utils_soft_labels import \
    clean_description  # need to add to pythonpath
...
def get_paper_classification_predictions(raw_text_column: pd.Series, top_k: int) -> pd.Series:
    ...
    predictions = clean_text_column.apply(partial(pipeline, top_k=top_k, truncation=True))
