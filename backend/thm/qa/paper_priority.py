...
class PriorityPapersManager:
    ...
    paper_ids: List[str]
    """List of ids of the papers associated to passed contexts."""
    joining_key: str = " "
    ...
    def find_paper(self, target_start: int, target_end: int, return_paper_idx: bool=False) -> Union[str, Tuple[str, int]]:
        """TODO
        ...
        if start_paper_idx != end_paper_idx: # TODO: handle case of multiple documents
            raise NotImplementedError("Cannot handle the case where the answer comes from two different papers.")
        else:
            paper_id = self.paper_ids[start_paper_idx]
            if return_paper_idx:
                return (paper_id, start_paper_idx)
            else:
                return paper_id
